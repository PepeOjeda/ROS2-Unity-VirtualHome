...
namespace RobotAtVirtualHome
{
    public class VirtualObject : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                var options = FindObjectOfType<EnvironmentManager>().m_simulationOptions;
                if (options.fixAllSeeds)
                    m_seed = 1;
                else
                {
                    m_seed = options.specifySeedByObjectType.Find(pair => tags.Contains(pair.objectTag)).seed;
                    if (m_seed <= 0)
                    {
